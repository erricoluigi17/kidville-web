  ...
  return (
    <div>
      <div className="flex flex-wrap items-end gap-3 mb-4">
        <div>
          <label className="font-maven text-xs text-gray-500 block mb-1">Data</label>
        ...
        {!sezione && sezioni && sezioni.length > 0 && (
          <div>
            <label className="font-maven text-xs text-gray-500 block mb-1">Sezione</label>
            <select value={filtroSezione} onChange={e => setFiltroSezione(e.target.value)}
              className="border-2 border-gray-200 rounded-lg px-3 py-1.5 font-maven text-sm text-kidville-green bg-white">
        ...
        {totaleConflitti > 0 && (
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-red-500 text-white animate-pulse">
      ...
      {error && <p className="font-maven text-sm text-red-500 mb-3">{error}</p>}
      {loading && <p className="font-maven text-sm text-gray-400">Caricamento…</p>}
        ...
        <div className="space-y-4">
          <div>
            ...
            {report.perClasse.length === 0 ? (
              <p className="font-maven text-sm text-gray-400">Nessuna prenotazione per questa data.</p>
            ) : (
              <div className="grid md:grid-cols-2 gap-3">
                {report.perClasse.map(c => (
                  <div key={c.classe} className="rounded-2xl border border-gray-100 bg-white overflow-hidden">
                    ...
                    <ul className="divide-y divide-gray-50">
                      {c.alunni.map(a => {
                        ...
                        return (
                          <li key={a.id} className={`px-3 py-2 ${conflitto ? 'bg-red-50' : ''}`}>
                            <div className="flex items-center justify-between gap-2">
                              <span className={`font-maven text-sm ${conflitto ? 'text-red-700 font-bold' : 'text-kidville-green'}`}>
                                {conflitto && <AlertTriangle size={12} className="inline mr-1 -mt-0.5 text-red-500" />}
                                {a.nome}
                              </span>
                                  ...
                                  {a.allergeni.map(k => {
                                    ...
                                    return (
                                      <span key={k} title={allergeneLabel(k)}
                                        className={`px-1.5 py-0.5 rounded-full font-maven text-[10px] font-bold ${inConflitto ? 'bg-red-500 text-white' : 'bg-gray-100 text-gray-500'}`}>
                            ...
                            {conflitto && (
                              <p className="font-maven text-[11px] text-red-500 mt-0.5">
