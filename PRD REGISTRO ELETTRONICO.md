...
| **P0 — RLS lockdown S9a + hardening (DL-038/039)** | — | RLS prod (default-deny anon; service-role passa) | — | ✅ S9a (migr. `20260752`): drop 8 permissive su tabelle service-role-only (incl. `valutazioni` = voti anon-esposti); revoca `exec_sql` da anon/authenticated; search_path su 12 funzioni. `get_advisors` **0 ERROR**, WARN 18→8. 🔶 S9b (8 tabelle session-client + chat realtime) + S13 sigillo = gated onboarding/per-famiglia (`P0_ROLLOUT_CHECKLIST.md`) |
