...
// Gli operatori delle condizioni: `value` è l'enum (non tradotto), `labelKey`
// è la chiave i18n dell'etichetta mostrata.
const OPERATORI: { value: FormFieldCondition['operator']; labelKey: string }[] = [
  { value: 'eq', labelKey: 'ppOpEq' },
  { value: 'neq', labelKey: 'ppOpNeq' },
  { value: 'contains', labelKey: 'ppOpContains' },
  { value: 'gt', labelKey: 'ppOpGt' },
  { value: 'lt', labelKey: 'ppOpLt' },
]
...
export function PropertiesPanel({ field, onChange, campiDisponibili = [] }: Props) {
  ...
  function addOption() {
    ...
    const newOpt: FormFieldOption = { label: t('ppOpzioneN', { n: idx }), value: `opt${idx}` }
        ...
        {field ? (
          ...
          >
            ...
            <div className="flex items-center gap-2 pb-3 border-b border-kidville-line">
              ...
              <span className="text-xs font-bold text-kidville-muted uppercase tracking-widest">{t('ppProprietaCampo')}</span>
            </div>
            ...
            <section className="flex items-center justify-between py-1">
              <div>
                <p className="text-sm font-medium text-kidville-ink">{t('obbligatorio')}</p>
                <p className="text-[11px] text-kidville-muted mt-0.5">{t('ppObbligatorioDesc')}</p>
              </div>
              <button
            ...
            {!HAS_OPTIONS.has(field.type) && field.type !== 'consent' && (
              <section className="space-y-1.5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-xs text-kidville-muted">{t('ppPunti')}</span>
                </div>
              </section>
            )}
                  ...
                  {(field.options ?? []).map((opt, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        title={t('ppPuntiGraduatoriaTitle')}
            ...
            <section className="space-y-2 pt-3 border-t border-white/[0.05]">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  style={{ width: 40, height: 22 }}
                  title={t('ppLogicaCondizionaleTitle')}
                >
                  <span
              ...
              {field.condition && (
                campiDisponibili.length === 0 ? (
                  <p className="text-[10px] text-kidville-muted leading-relaxed">
                    {t('ppCondizioneNoCampi')}
                  </p>
                ) : (
                  <div className="space-y-2">
                    <p className="text-[10px] text-kidville-muted">{t('ppMostraCampoSe')}</p>
                    <select
                    ...
                    >
                      {OPERATORI.map(o => (
                        <option key={o.value} value={o.value} className="bg-kidville-ink">{t(o.labelKey)}</option>
                      ))}
                    </select>
                    <input
            ...
            <div className="pt-3 border-t border-white/[0.05]">
              ...
              <p className="text-[10px] text-kidville-muted mt-0.5">
                {t('ppId')} <span className="font-mono text-kidville-muted">{field.id.slice(0, 8)}…</span>
              </p>
            </div>
          </motion.div>
        ) : (
          ...
          >
            ...
            <p className="text-sm font-medium text-kidville-muted">{t('ppNessunCampo')}</p>
