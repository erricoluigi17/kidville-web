...
| `KV_SEED_CONFERMO` | **Attrito, non configurazione.** `scripts/seed_parents.ts` rifiuta di partire se non vale `1`. Quello script scrive con la **service role key** sul database indicato da `NEXT_PUBLIC_SUPABASE_URL` — che in questo repo, con `.env.local`, è la **PRODUZIONE**, dove ci sono dati reali di minori: un `npx tsx scripts/seed_parents.ts` battuto per sbaglio era una scrittura silenziosa su prod. **Non va impostata da nessuna parte**, né in locale né in produzione: si scrive davanti al comando nel momento in cui si decide di eseguirlo. Non è un segreto. L'app non la legge mai. |
