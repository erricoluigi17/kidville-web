...
> Documento per la sessione che riprende il lavoro. Stato aggiornato al **23/09/2026 21:28**.
...
| **Consegna 2a** (NON ancora in produzione) | PR da aprire, branch `fix/coda-fatture-rilievi-nucleo` | — | Rilievi (b)–(e) di §3: «Togli» azzera anche l'esito (migrazione `20260923191725_fatture_coda_togli_azzera_esito.sql`, la applica l'integrazione al merge); pausa e fine stimata col giorno; via il codice morto del vecchio lotto e le 18 chiavi `reconLotto*`; chip della coda sulle righe di Pagamenti e Riconciliazione. Piano: `consegna-2a-rilievi.md` |

**Misurato al 23/09 alle 21:26** (solo `SELECT` in produzione): `fatture_coda` ha **0 voci**, e l'ultima migrazione applicata è ancora `20260923102831` (il nucleo). Il cardine (decisione 0: una fattura accodata parte dal lavoratore anche a PC spento) non l'ha ancora provato nessuna fattura vera: la prova dal vivo resta da fare con le prime fatture della segreteria (e con quelle anche il chip sulle righe e la stima col giorno).
...
Sei punti verificati al 23/09. I primi cinque, (a)-(e), riguardano il nucleo così com'è in produzione oggi — nessuno blocca il rilascio già fatto. **(b), (c), (d) ed (e) sono chiusi dalla consegna 2a** (branch `fix/coda-fatture-rilievi-nucleo`, PR da aprire, non ancora in produzione; piano in `consegna-2a-rilievi.md`). Resta aperto (a), da chiudere nella seconda consegna. Il sesto, (f), non è del nucleo: è un residuo che la fotografia rigenerata ha fatto vedere, e lo decide il titolare:
...
- **(b)** ✅ **Chiuso nella consegna 2a**: la migrazione `20260923191725_fatture_coda_togli_azzera_esito.sql` rifà `fatture_coda_togli` identica a quella del nucleo più `esito_codice = NULL, esito_messaggio = NULL`, e un blocco `DO` ripulisce le voci già tolte (0 il 23/09); test su PGlite in `__tests__/db/fatture-coda-nucleo.test.ts`. La applica l'integrazione al merge, mai a mano. Il rilievo com'era: **`esito_messaggio` può contenere il nome di un genitore e resta dopo «Togli»**: la RPC `fatture_coda_togli` (righe 571-582 della migrazione `20260923102831_fatture_coda_nucleo.sql`) azzera `causale_manuale` e `intestatario_scelto`, ma **non** `esito_codice`/`esito_messaggio` — a differenza di `fatture_coda_rimetti`, che li azzera entrambi. Una voce tolta dopo un errore può quindi restare con un `esito_messaggio` che cita un nome in chiaro (messaggi d'errore Aruba/SDI). Da correggere: aggiungere `esito_codice = NULL, esito_messaggio = NULL` a `fatture_coda_togli`.
...
9. Dei rilievi elencati sopra resta aperto solo (a), l'intestatario «Altro» fuori dalla coda; il (f) aspetta prima la decisione del titolare. (b)–(e) li chiude la consegna 2a. Le domande che la consegna 2a lascia al titolare (il «chi» della «Togli», il codice del motivo su una voce tolta, la formula della stima, il chip nel popup e nel dettaglio, «Invia fattura» sulle righe già in coda, e altre) sono in `consegna-2a-rilievi.md` §7.2.
