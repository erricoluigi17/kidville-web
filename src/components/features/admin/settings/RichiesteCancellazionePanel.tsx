  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 p-6 font-maven text-sm text-kidville-muted">
        <Loader2 className="animate-spin" size={16} /> Caricamento richieste…
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-3 rounded-2xl border border-kidville-warn/30 bg-kidville-warn-soft p-4">
        ...
        <p className="font-maven text-[13px] leading-relaxed text-kidville-ink/80">
          Richieste avviate dai <strong>genitori</strong> dall’app (Profilo e deleghe). Evaderle
          <strong> anonimizza</strong> il genitore e i figli <strong>non iscritti</strong> (irreversibile);
          i figli ancora iscritti e i documenti fiscali restano. Da evadere <strong>entro 30 giorni</strong> (GDPR).
        </p>
      </div>
      ...
      {list.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-kidville-line bg-kidville-white/60 p-10 text-center">
          ...
          <p className="mt-2 font-maven text-sm text-kidville-muted">Nessuna richiesta di cancellazione in attesa.</p>
        </div>
      ) : (
              ...
              return (
                ...
                >
                  ...
                  <span className="truncate font-maven text-[11.5px] text-kidville-muted">
                    Figli: {r.alunni_non_iscritti} non iscritti · {r.alunni_iscritti} iscritti (mantenuti)
                  </span>
                </button>
              );
            })}
          ...
          <section className="rounded-2xl border-t-4 border-kidville-error bg-kidville-white p-5 shadow-sm" style={{ boxShadow: '0 1px 3px rgba(0,84,75,.04), 0 8px 24px -18px rgba(0,84,75,.28)' }}>
            {!target ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="mt-2 font-maven text-sm text-kidville-muted">Seleziona una richiesta per evaderla.</p>
              </div>
            ) : (
              <>
                <h3 className="flex items-center gap-2 font-barlow text-xl font-black uppercase tracking-wide text-kidville-error">
                  <AlertTriangle size={20} /> Evadi la cancellazione
                </h3>
                <p className="mb-4 mt-2 font-maven text-sm text-kidville-ink/80">
                  Stai per <strong>anonimizzare irreversibilmente</strong> l’account di <strong>{target.parent_nome}</strong>.
                </p>
                ...
                {busy && !dry ? (
                  ...
                  <div className="mb-4 space-y-1 rounded-xl bg-kidville-cream p-3.5 font-maven text-xs text-kidville-ink/80">
                    <div>Genitore anonimizzato: <strong>{dry.parent}</strong></div>
                    <div>Figli non iscritti anonimizzati: <strong>{dry.alunni_non_iscritti}</strong></div>
                    {dry.alunni_iscritti_mantenuti > 0 && <div className="text-kidville-warn">Figli iscritti mantenuti: {dry.alunni_iscritti_mantenuti}</div>}
                  </div>
                ) : null}
