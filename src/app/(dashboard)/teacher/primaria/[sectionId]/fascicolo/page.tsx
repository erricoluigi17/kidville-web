...
import { FolderLock, Upload, Download, ShieldAlert } from 'lucide-react';
...
export default function FascicoloPage() {
  ...
  const loadDocs = useCallback(async () => {
    if (!alunnoId) { setDocs([]); setDenied(false); return; }
    ...
    <div className="space-y-4">
      <div className="rounded-card bg-white p-5 shadow-sm">
        ...
        <p className="font-maven text-xs text-gray-400 mb-3">
          Documenti riservati (PEI/PDP/sanitari). Accesso limitato ai docenti contitolari e alla dirigenza; ogni accesso è tracciato.
        </p>
      ...
      {alunnoId && !denied && (
        <>
          ...
          <div className="rounded-card bg-white p-5 shadow-sm">
            <h3 className="font-barlow text-base font-bold text-gray-800 mb-3">Documenti</h3>
            {docs.length === 0 ? (
