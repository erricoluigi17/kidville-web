...
| Notifiche SdI | ✅ `arubaGetNotifications` **corretta e in uso** dal 2026-09-11 | Parametro documentato: **`invoiceFilename`** · **A** (docs v1 §11.2) | `client.ts` invia `invoiceFilename`; chiamata da `pagamenti/fattura/sync` quando uno scarto non porta il motivo | **Divergenza chiusa.** Mandava `filename`, ed era «dormiente» finché nessuno la chiamava. Si è svegliata il 2026-09-11: `getByFilename` NON dà il motivo di uno scarto (misurato su 4 fatture respinte vere, tutte con `statusDescription`/`errorCode`/`errorDescription` vuoti), quindi le notifiche sono diventate l'unica strada. ⚠️ La **forma della risposta** non è mai stata misurata contro l'API vera: l'estrattore è un'euristica difensiva e, quando non riconosce nulla, scrive in `app_log` i **nomi** dei campi (mai i valori — il corpo porta denominazione, CF e P.IVA di una famiglia). Vedi §5 |
...
| **2** | **Notifiche SdI** (`arubaGetNotifications`) | ✅ **Fatto il 2026-09-11**: parametro corretto in `invoiceFilename` e un chiamante c'è (§1.5) | Restava qui come «non oggi» perché «lo stato si legge da `getByFilename`, che basta». **Non bastava**: il 2026-09-11 le prime 4 fatture respinte vere sono arrivate a registro con «Aruba: «Scartata» — nessun motivo dal provider», cioè lo stato senza il perché, che è il dato con cui la Segreteria corregge e ritrasmette. Ciò che resta aperto non è più il nome del parametro ma la **forma della risposta**, che non si può misurare senza una notifica reale: alla prima, la riga `forma notifiche SDI` in `app_log` porta i nomi dei campi e permette di sostituire l'euristica con una misura |
