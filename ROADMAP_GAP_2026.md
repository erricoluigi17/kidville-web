...
- **Modulistica & Form (Fase 4, builder 23/40 + modulistica 11/33)**:
  - **Form Builder Drag & Drop** completo (blocchi Bambino/Adulto, ~~Consensi/Allegati~~ ✅ **FATTO (P3.3e, DL-029)**), ~~**logica condizionale**~~ ✅ **FATTO (P3.3a, DL-024)** (motore `conditional.ts`: wizard mostra/nasconde + valida solo visibili + strip; editor nel builder), scoring/soglia graduatoria, "Pubblica modello", config accessi (registrati/link pubblico).
    - ✅ **P3.3e (DL-029)**: blocco **Consensi/Privacy** (tipo `consent`: testo+link+checkbox obbligatoria) + **snapshot legale** `consents_log` (migr. `20260746`); blocco **Allegati** con `accept`/`max_size_mb`; **upload generico** `/api/forms/upload` (ripara wizard autenticato) + `/api/forms/submit` (insert server-role); **gate** `requireStaff` su `/api/admin/form-models` (era ungated). *(Restano "Pubblica modello" + config accessi + upload pubblico token-scoped.)*
