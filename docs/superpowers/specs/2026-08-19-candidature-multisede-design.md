...
candidature_sedi                       -- LA CANDIDATURA A UN PLESSO: una riga per sede
...
**`scuola_id` sul figlio DEVE dichiarare la sua FK verso `schools(id)` nella stessa
...
legge senza sapere del figlio.

### Il trigger: `stato` della madre come aggregato

`candidature_insegnanti.stato` smette di essere scritto a mano e diventa la sintesi dei
figli, mantenuta da un trigger `after insert or update or delete on candidature_sedi`:

| Figli | Madre |
|---|---|
| almeno uno `pending` | `pending` |
| nessuno `pending`, almeno una `approvata` | `approvata` |
| tutti `rifiutata` | `rifiutata` |
...
la persona **è ancora in gioco**: la madre resta `pending`, l'indice continua a dire
...
`in_approvazione` **non** compare fra gli stati del figlio: il claim in due tempi è
morto il 2026-08-15, quando approvare ha smesso di creare account e di spedire
password. Resta solo nel `check` della madre perché lo storico lo contiene.
...
- si scrive la madre (con `scuola_id` = il primo dell'elenco) e poi i figli.

**Degrado sul DB E2E della CI, che non è migrato.** Se `candidature_sedi` non esiste
(`42P01` / `PGRST205`), la candidatura si registra **comunque** sulla riga madre con la
prima sede e si logga un `warn` con esito `sedi-multiple-non-registrate`. Il `201` resta
...
- **elenco e dettaglio** filtrano per sede **attraverso il figlio**
  (`candidature_sedi.scuola_id in (scuole attive)`), non più sulla madre.
...
- **il gate anti-IDOR sul curriculum** (`assertCurriculumInScope`) passa dal figlio.
  Se restasse sulla madre, una segreteria di Aversa non vedrebbe il curriculum di una
...
- **l'email di esito nomina già la sede** (`La Segreteria di ${sede.nome}`). Con il
  ...
  della sede che ha rifiutato, non quello della madre.
...
`?sede=<uuid>` continua a valere come oggi: preseleziona **quella** sede, ma — questa è
la differenza — l'elenco resta visibile e se ne possono aggiungere altre. Oggi il
parametro **salta** il passo; da domani lo apre già spuntato. Un link targato che
impedisse di candidarsi anche altrove sarebbe una restrizione che nessuno ha chiesto.
...
La voce nella privacy va quindi aggiornata perché dica il vero, ed è parte di questo
lavoro, non un seguito: un'informativa che descrive una cancellazione che non avviene è
