    ...
    <div className="space-y-2">
      ...
      {field.type === 'file' && (
        <Controller
          ...
          defaultValue=""
          render={({ field: rhf }) => (
            <FileField modelId={modelId} value={rhf.value} onChange={rhf.onChange} />
          )}
        />
      )}
...
// ── Upload allegato su bucket form_attachments ───────────────
function FileField({
  modelId,
  value,
  onChange,
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const path = `${modelId}/${crypto.randomUUID()}-${safeName}`
