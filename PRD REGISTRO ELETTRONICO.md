...
| **P3 — Delibera ammissioni + scoring (DL-025)** | `requireStaff` (delibera/override) | per `model_id` | `esito_da`/`esito_il` su `form_submissions` | ✅ Fatto (P3.3b): scoring applicato in live (migr. `20260743`), `calcolaDelibera` (soglia+posti), esito ammesso/lista/non + override, export PDF delibera, UI RankingTable. ⚠️ trigger ETL form→anagrafiche deferito (drift `adults`/`student_adults`) |
