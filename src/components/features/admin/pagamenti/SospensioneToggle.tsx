        ...
        <>
            ...
            {open && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => !busy && setOpen(false)}>
                    <div className="w-full max-w-md rounded-[22px] bg-kidville-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
                        <div className="mb-3 flex items-center justify-between">
                            <h2 className="font-barlow text-lg font-black uppercase text-kidville-error-strong">Sospensione morosità</h2>
                            <button onClick={() => !busy && setOpen(false)} className="text-kidville-muted hover:text-kidville-error" aria-label="Chiudi">
                                <X size={18} />
                            </button>
                        </div>

                        <p className="mb-2 font-maven text-[12.5px] text-kidville-muted">
                            La sospensione vale per l’intera famiglia. Verranno coinvolti:
                        </p>
                        <div className="mb-4 rounded-[14px] bg-kidville-cream px-3 py-2">
                            {caricando ? (
                                <div className="flex items-center gap-2 py-1 font-maven text-[12px] text-kidville-muted">
                                ...
                                <ul className="space-y-1">
                                    {figli.map((f) => (
                                        <li key={f.id} className="flex items-center justify-between font-maven text-[13px] text-kidville-green">
                                            ...
                                            {f.sospeso && <span className="font-barlow text-[10px] font-bold uppercase text-kidville-error">già sospeso</span>}
                                        </li>
                                    ))}
                                </ul>
                            ) : (
                                <p className="py-1 font-maven text-[12px] text-kidville-muted">Solo questo alunno (nessun altro figlio collegato).</p>
                            )}
                        </div>

                        <label className="mb-1 block font-barlow text-[11px] font-bold uppercase tracking-wide text-kidville-green">Causa</label>
                        <div className="mb-4 flex gap-2">
                            {(['morosita', 'altro'] as const).map((c) => (
                                <button
                                    ...
                                    type="button"
                                    onClick={() => setCausa(c)}
                                    className={`flex-1 rounded-[12px] px-3 py-2 font-maven text-[13px] font-semibold ${causa === c ? 'bg-kidville-green text-kidville-white' : 'bg-kidville-cream text-kidville-green'}`}
                                >
                        ...
                        <div className="flex justify-end gap-2">
                            <button onClick={() => setOpen(false)} disabled={busy} className="rounded-[12px] px-4 py-2 font-maven text-[13px] font-semibold text-kidville-muted disabled:opacity-50">
                                Annulla
                            </button>
