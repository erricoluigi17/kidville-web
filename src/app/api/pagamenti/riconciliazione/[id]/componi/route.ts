...
// eleganza: sul riabbinamento di un movimento riaperto si affacciano DUE rotte
...
//
// ─── COSA PORTA QUESTA ROTTA, CHE LA RPC PER COSTRUZIONE NON PUÒ AVERE ──────
...
// stesso branch col pulsante d'annullo, e questa rotta è la sua SECONDA porta:
// la prima (`riconciliazione/[id]:PATCH`) le legge entrambe.
//
...
//
//  6. I TETTI STANNO NELLA `zod`, e sono misurati — vedi `MAX_*` più sotto.
//
 ...
 * non torna.
 */
const MAX_QUANTITA_TICKET = 2_147_483_647
const MAX_IMPORTO_EURO = 1_000_000
const MAX_RIGHE_PER_ELENCO = 50
const MAX_RIGHE_TOTALI = 60
...
// resta per le venti righe di log che la nominano. Che le due stringhe restino la
// stessa lo verifica un test («il nome del withRoute e la costante OPERAZIONE non
// divergono»), non la buona volontà di chi rinomina.
  ...
  async (request: Request, context: { params: Promise<{ id: string }> }) => {
    try {
      ...
      if (errMov) {
        ...
        if (code === COLONNA_ASSENTE) {
          ...
          return NextResponse.json(
            { error: 'La composizione del pagamento non è disponibile su questo ambiente: nessun incasso è stato registrato.', codice: 'CONCILIAZIONE_NON_DISPONIBILE' },
            { status: 503 },
          )
        }
        logErrore({ operazione: OPERAZIONE, stato: 500, evento: 'db' }, errMov)
        return NextResponse.json(
          { error: 'Movimento non leggibile', codice: 'CONCILIAZIONE_CONTESTO_NON_LETTO' },
          { status: 500 },
        )
      }
      const movimento = movRaw as MovimentoRiga | null
      if (!movimento) {
        logEvento('pagamento', 'info', { operazione: OPERAZIONE, esito: 'movimento-inesistente', movimento_id: movimentoId })
        return NextResponse.json(
      ...
      if (!STATI_CONCILIABILI.has(statoAtteso)) {
        ...
        return NextResponse.json(
          { error: 'Questo bonifico non è più nello stato in cui lo si stava componendo', codice: 'CONCILIAZIONE_MOVIMENTO_CAMBIATO' },
          { status: 409 },
        )
      }
      ...
      // route è più esposta: una cosa validata contro un insieme e la sua gemella
      // contro un altro.
      //
      ...
      // Il prezzo è che una selezione stretta nel
      // SedeSelector restringe anche la scrittura — e la direzione in cui si
      // sbaglia è quella sicura: mai più larga dell'ambito dichiarato a schermo.
      ...
      // questa route: quelli parlano di righe che il chiamante ha NOMINATO lui
      ...
      // questa route, altrimenti il rifiuto è muto.
      //
      ...
      if (pagamentoDiPrima != null && !body.voci.some((v) => v.pagamento_id === pagamentoDiPrima)) {
        ...
        if (errFatture) {
          logErrore(
            { operazione: OPERAZIONE, stato: 503, evento: 'fatture_del_movimento_non_lette' },
            errFatture,
          )
          return NextResponse.json(
            {
      ...
      if (idVoci.length > 0) {
        ...
        if (ris.error && ris.error.code === COLONNA_ASSENTE) {
          logEvento('pagamento', 'warn', {
            operazione: OPERAZIONE, esito: 'sconto-non-disponibile', tipo: 'colonna-sconto',
          }, ris.error)
        ...
        if (ris.error) {
          ...
          logErrore({ operazione: OPERAZIONE, stato: 500, evento: 'db' }, ris.error)
          return NextResponse.json(
            { error: 'Verifica delle voci non riuscita', codice: 'CONCILIAZIONE_CONTESTO_NON_LETTO' },
            { status: 500 },
          )
        }
      ...
      if (mancanti.length > 0) {
        logEvento('pagamento', 'info', {
          operazione: OPERAZIONE, esito: 'voci-inesistenti', movimento_id: movimentoId, n: mancanti.length,
        })
        return NextResponse.json(
          { error: 'Una o più voci non esistono più', codice: 'PAGAMENTO_NON_TROVATO' },
          { status: 404 },
        )
      }
      ...
      if (vociFuoriSede.length > 0) {
        logEvento('pagamento', 'warn', {
          operazione: OPERAZIONE, esito: 'voci-fuori-sede', tipo: 'voce-esistente',
          movimento_id: movimentoId, n: vociFuoriSede.length,
        })
        return NextResponse.json(
          { error: 'Una o più voci appartengono a un plesso che non puoi gestire', codice: 'CONCILIAZIONE_SEDE_NON_ACCESSIBILE' },
          { status: 403 },
        )
      }
      ...
      if (vociContenitore.length > 0) {
        logEvento('pagamento', 'warn', {
          operazione: OPERAZIONE, esito: 'voci-contenitore',
      ...
      if (idAlunniNuovi.length > 0) {
        ...
        if (ris.error && ris.error.code === COLONNA_ASSENTE) {
          ...
          logEvento('pagamento', 'warn', {
            operazione: OPERAZIONE, esito: 'oblio-non-verificabile', tipo: 'colonna-anonimizzato-il',
          }, ris.error)
        ...
        if (ris.error) {
          logErrore({ operazione: OPERAZIONE, stato: 500, evento: 'db' }, ris.error)
          return NextResponse.json(
            { error: 'Verifica dei bambini non riuscita', codice: 'CONCILIAZIONE_CONTESTO_NON_LETTO' },
            { status: 500 },
          )
        }
      ...
      if (alunniMancanti.length > 0) {
        logEvento('pagamento', 'info', {
          operazione: OPERAZIONE, esito: 'alunni-inesistenti', movimento_id: movimentoId, n: alunniMancanti.length,
        })
        return NextResponse.json(
          { error: 'Uno o più bambini non sono più raggiungibili', codice: 'ALUNNO_NON_APRIBILE' },
          { status: 404 },
        )
      }
      ...
      if (idSedi.length > 0) {
        ...
        if (errScuole) {
          logEvento('pagamento', 'warn', {
            operazione: OPERAZIONE, esito: 'nomi-sedi-non-letti', tipo: 'riconoscimento-e2e-parziale',
          }, errScuole)
        } else {
      ...
      if (alunniFuoriSede.length > 0) {
        logEvento('pagamento', 'warn', {
          operazione: OPERAZIONE, esito: 'alunni-fuori-sede', tipo: 'voce-nuova',
          movimento_id: movimentoId, n: alunniFuoriSede.length,
        })
        return NextResponse.json(
          { error: 'Una o più voci riguardano un plesso che non puoi gestire', codice: 'CONCILIAZIONE_SEDE_NON_ACCESSIBILE' },
          { status: 403 },
        )
      }
      ...
      if (alunniNonAttivi.length > 0) {
        ...
        logEvento('pagamento', 'warn', {
          operazione: OPERAZIONE, esito: 'alunni-non-attivi', tipo: 'voce-nuova',
          movimento_id: movimentoId, n: alunniNonAttivi.length,
        })
        return NextResponse.json(
          { error: 'Una o più voci riguardano un bambino che non è più fra gli iscritti', codice: 'CONCILIAZIONE_ALUNNO_NON_ATTIVO' },
          { status: 403 },
        )
      }
      ...
      if (body.ancora) {
        ...
        if (indice >= dimensione) {
          logEvento('pagamento', 'info', {
            operazione: OPERAZIONE, esito: 'ancora-fuori-elenco', tipo: specie, movimento_id: movimentoId,
          })
          return NextResponse.json(
            { error: 'La voce indicata come àncora della fattura non è nella composizione', codice: 'CONCILIAZIONE_ANCORA_MANCANTE' },
            { status: 400 },
          )
        }
        ...
        // `catch` in fondo, cioè un 500 opaco — e sarebbe la prima volta che
        // questa rotta risponde 500 prima di aver scritto un centesimo, con la
        ...
        if (!proposta) {
          ...
          return NextResponse.json(
            { error: 'Nessuna voce da cui intestare la fattura', codice: 'CONCILIAZIONE_ANCORA_MANCANTE' },
            { status: 400 },
          )
        }
      ...
      // nella testata della migrazione, che chiede espressamente a questa route di
      // mandare `[]`.
      const payload = {
        ...
        // Sempre `[]`, anche se questa rotta non lo popola mai: la ricarica SENZA
      ...
      if (rpcErr) {
        ...
        if (RPC_ASSENTE.has(code)) {
          logEvento('pagamento', 'warn', {
            operazione: OPERAZIONE, esito: 'rpc-assente', error_code: code, movimento_id: movimentoId,
          }, rpcErr)
          return NextResponse.json(
            { error: 'La composizione del pagamento non è disponibile su questo ambiente: nessun incasso è stato registrato.', codice: 'CONCILIAZIONE_NON_DISPONIBILE' },
            { status: 503 },
        ...
        if (code === CAS_PERSO) {
          logEvento('pagamento', 'warn', {
            operazione: OPERAZIONE, esito: 'movimento-gia-conciliato', error_code: code,
            movimento_id: movimentoId, stato: statoAtteso,
          })
          return NextResponse.json(
            { error: 'Un altro operatore ha già conciliato questo bonifico', codice: 'CONCILIAZIONE_MOVIMENTO_CAMBIATO' },
            { status: 409 },
          )
        }
        ...
        logErrore({ operazione: OPERAZIONE, stato: 500, evento: 'rpc' }, rpcErr)
        return NextResponse.json(
          { error: 'Non siamo riusciti a registrare il pagamento: nulla è stato scritto.', codice: 'CONCILIAZIONE_NON_REGISTRATA' },
          { status: 500 },
        )
      }
      ...
      // insieme senza che nessun test se ne accorga.
      if (alunniCoinvolti.length > 0) {
        try {
