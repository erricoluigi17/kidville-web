                ...
                {!sel ? <p className="font-maven text-sm text-gray-400">Seleziona un alunno.</p> : (
                    <div className="bg-kidville-cream/60 rounded-xl p-4">
                        <div className="flex justify-between mb-3">
                            ...
                            <span className="font-maven text-sm text-gray-500">Saldo: <b className="text-kidville-green">{saldo ?? '—'}</b> ticket</span>
                        </div>
                        {pacchetti.length > 0 && (
                            <div className="flex flex-wrap gap-2 mb-3">
                                {pacchetti.map((p, i) => (
                                    <button key={i} onClick={() => { setPezzi(p.pezzi); setCosto(p.costo); }}
                                        className="px-3 py-1 rounded-full border-2 border-gray-200 font-maven text-xs text-kidville-green hover:border-kidville-green">
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-3">
                            <div><label className="font-maven text-xs text-gray-500">Pezzi</label>
                                <input type="number" value={pezzi} onChange={e => setPezzi(Number(e.target.value))} className="w-full border-2 border-gray-200 rounded-lg px-2 py-1 font-maven text-sm text-kidville-green" /></div>
                            <div><label className="font-maven text-xs text-gray-500">Costo €</label>
                                <input type="number" value={costo} onChange={e => setCosto(Number(e.target.value))} className="w-full border-2 border-gray-200 rounded-lg px-2 py-1 font-maven text-sm text-kidville-green" /></div>
                            <div><label className="font-maven text-xs text-gray-500">Metodo</label>
                                <select value={metodo} onChange={e => setMetodo(e.target.value)} className="w-full border-2 border-gray-200 rounded-lg px-2 py-1 font-maven text-sm text-kidville-green bg-white">
                        ...
                        {done && <p className="mt-2 font-maven text-xs text-green-600 flex items-center gap-1"><CheckCircle2 size={13} /> {done}</p>}
                    </div>
                )}
            </div>
        </div>
