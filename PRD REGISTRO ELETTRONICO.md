...
- **Sequenziamento/logging**: branch nuovo da `main` in worktree (il branch `feat/logging-strutturato` mergerà dopo); su questo branch **non** esiste l'infrastruttura logging (`withRoute`/`logger`) → le nuove superfici hanno un'**appendice di adeguamento** da applicare alla convergenza (dettaglio nello spec: 422 privacy-lock → `logEvento` coi soli conteggi, pubblicazione ok → `logOk`, errori storage → `logErrore` col corpo; mensa prenotazione/disdetta → `logOk` con esiti+`saldoDopo`+origine, saldo negativo → `logEvento` info).
- **Gate**: eslint **0** · tsc **0** · vitest **1229 / 196 file** verdi · build + E2E in CI al push.
