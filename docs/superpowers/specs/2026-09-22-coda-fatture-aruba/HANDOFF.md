...
- [ ] **Registrare le orfane** (7 al 23/09 12:00; ricontarle): `node scripts/fatture-orfane.mjs` prima a secco, poi `--solo <id>`, poi `--applica`, sempre **mostrando** le scritture. **Il nucleo è attivo e la coda di default non è sospesa**: senza preparazione lo script si ferma con `coda-attiva`. Prima di lanciarlo un admin **sospende la coda** (pagina «Coda fatture», oppure la RPC `fatture_coda_sospendi`), aspetta che il testimone del lavoratore scada (altrimenti `cancello-in-uso`, col suo orario) e che finisca un'eventuale pausa da 429 o da esito incerto (altrimenti `circuito-aperto`, col suo orario); a fine lavoro **riprende la coda**. Meglio a segreteria ferma. Procedura: `scomposizione.md` § R1-10.1. **Rimandato alla fase B2 di questa PR**: gira lì, non nel nucleo appena rilasciato — questa riga va aggiornata dopo.
- [ ] **Indagine sul salto FPR 2154→2516**: `node scripts/numerazione-serie.mjs --out $LAVORO/indagine-r1 --xml-fuori-app`, solo prima delle 08:00 o dopo le 18:30, e con la **stessa preparazione** delle orfane: coda sospesa da un admin, testimone del lavoratore scaduto, nessuna pausa in corso, e coda ripresa a fine lavoro (altrimenti la guardia si ferma con `coda-attiva`, `cancello-in-uso` o `circuito-aperto`). Criteri P0–P7 in `d1-correzione-urgente.md` §3; ipotesi favorita H1: numeri scritti a mano sul pannello da segreteria o commercialista. **Rimandata alla fase B2 di questa PR**: gira lì, non nel nucleo appena rilasciato — questa riga va aggiornata dopo.
