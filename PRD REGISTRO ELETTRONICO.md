...
- **Schema (2 migrazioni additive, prod advisors 0 ERROR):** `20260721100000_news_base` (5 tabelle: `news_categorie` con 5 categorie di sistema seed, `news_posts` con `search_tsv` GENERATED + indici parziali, `news_media`, `news_visualizzazioni`, `news_digest_edizioni`; RLS abilitata su tutte, **nessuna** policy INSERT/UPDATE/DELETE per `authenticated` — le mutazioni passano solo dalle route service-role; SELECT difensive fail-closed) e `20260721101000_news_cron` (2 funzioni `SECURITY DEFINER` + `SET search_path`, **REVOKE da PUBLIC, anon, authenticated** + GRANT a service_role, job `news-tick` `*/10 * * * *` e `news-digest` `0 8 1 * *`).
