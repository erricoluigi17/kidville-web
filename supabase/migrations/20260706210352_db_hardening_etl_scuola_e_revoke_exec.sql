-- 20260706210352 — Hardening DB post-deploy:
-- (a) fn_form_submission_etl NON hardcoda più la sede '11111111-...' (inesistente →
--     FK alunni_scuola_id_fkey falliva e l'alunno non veniva mai creato): la risolve
--     da public.schools (mono-sede in prod). Se nessuna sede → skip pulito.
-- (b) Riduce la superficie RPC delle SECURITY DEFINER non destinate ai client.
-- Idempotente: CREATE OR REPLACE + REVOKE/GRANT.
-- NB: versione allineata a quella registrata in prod da apply_migration.
...
BEGIN
  ...
  -- Sede di destinazione risolta dinamicamente (NIENTE UUID hardcoded): mono-sede
  -- in prod. FK alunni.scuola_id -> schools(id): senza sede valida non si inserisce.
  SELECT id INTO c_scuola_id FROM public.schools ORDER BY id LIMIT 1;
  IF c_scuola_id IS NULL THEN RETURN NEW; END IF;
...
-- (b) Superficie RPC ridotta. fn_form_submission_etl è solo trigger (nessun ruolo
-- client la esegue). notifiche_dispatch_tick/rls_auto_enable/mensa_check_allergie
-- non sono trigger né in RLS né chiamate via .rpc dall'app (cron/server). service_role
-- mantenuto per sicurezza server-side.
...
REVOKE EXECUTE ON FUNCTION public.notifiche_dispatch_tick() FROM PUBLIC, anon, authenticated;
GRANT  EXECUTE ON FUNCTION public.notifiche_dispatch_tick() TO service_role;
REVOKE EXECUTE ON FUNCTION public.rls_auto_enable() FROM PUBLIC, anon, authenticated;
GRANT  EXECUTE ON FUNCTION public.rls_auto_enable() TO service_role;
REVOKE EXECUTE ON FUNCTION public.mensa_check_allergie_giornaliero() FROM PUBLIC, anon, authenticated;
GRANT  EXECUTE ON FUNCTION public.mensa_check_allergie_giornaliero() TO service_role;

-- is_staff_or_admin: usata SOLO in policy RLS 'TO authenticated' → tolgo ad anon;
-- authenticated e service_role la mantengono (serve a valutare le policy).
