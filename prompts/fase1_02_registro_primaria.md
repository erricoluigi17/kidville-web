...
2. **Backend:** Crea la logica per il calcolo asincrono delle medie (sull'associazione numerica nascosta dei giudizi sintetici) e il job ritardato (10 min) per l'invio delle notifiche delle valutazioni. **La media numerica è riservata al personale docente/segreteria: NON deve mai comparire nel payload di alcun endpoint `/api/parent/**`, e gli endpoint docente che la restituiscono (es. prospetto) devono avere un gate di ruolo che escluda il genitore** (vedi PRD §4 #3 e §4.5).
3. **UI Docente:** Sviluppa la griglia oraria, modale per firma, form inserimento valutazioni e assegnazione note massiva.
4. **UI Genitore:** Sviluppa le sezioni per visualizzare orario, compiti, andamento scolastico e il flusso di "Firma per presa visione" delle note. **L'app genitore mostra esclusivamente i giudizi sintetici/descrittivi: non deve MAI renderizzare la media numerica (niente `media.toFixed(...)`), che resta uno strumento del docente** (PRD §4 #3 e §4.5).
