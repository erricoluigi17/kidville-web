...
/**
 * ── LA NORMALIZZAZIONE VIVE IN `@/lib/ui/testo-ricerca` (2026-09-01) ─────────
 *
 * `normalizzaTesto` e `rangoDiMatch` erano dichiarate qui. Sono servite
 * IDENTICHE alla ricerca della barra filtri riusabile, e ricopiarle sarebbe
 * stato il modo di farle divergere: la prima cosa che si aggiusta in una ricerca
 * è proprio la normalizzazione — un apostrofo in più, un trattino — e
 * correggerla in un posto su due lascia due campi che «cercano» in modo diverso
 * dentro la stessa pagina.
 *
 * Il ri-esporto NON è una cortesia verso i chiamanti: altri moduli le importano
 * da qui e il banco di prova di questo componente le misura per nome. Che siano
 * lo STESSO oggetto funzione — e non due che si somigliano — è verificato in
 * `__tests__/lib/filtri-motore.test.ts` §13.
 */
export { normalizzaTesto, rangoDiMatch };
