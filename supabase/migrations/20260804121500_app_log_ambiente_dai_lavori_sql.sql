...
--    mostrata al titolare: in produzione ci sono 227 domande d'iscrizione e 152 codici
--    fiscali di minori.
-- ═══════════════════════════════════════════════════════════════════════════════
--
...
-- 1. L'ambiente del DATABASE, dichiarato una volta e per sempre sul database stesso.
--    Dentro Postgres non c'è `VERCEL_ENV`: l'unica cosa che il DB può sapere di sé è ciò che
--    gli si dice. Il valore è per-database, quindi il progetto E2E della CI — che è un
--    database SEPARATO e non riceve questa migrazione — non erediterà mai 'production'
--    per sbaglio: lì il parametro resta assente e il ripiego dice la verità (vedi punto 2).
--    `ALTER DATABASE ... SET` agisce sulle CONNESSIONI FUTURE: i worker di pg_cron aprono una
--    connessione nuova a ogni tick, quindi il primo tick dopo il deploy lo vede già.
DO $$
BEGIN
  EXECUTE format('ALTER DATABASE %I SET app.ambiente = %L', current_database(), 'production');
END $$;

-- 2. Il DEFAULT. `current_setting(..., true)` non lancia quando il parametro non esiste:
--    restituisce NULL, e il `NULLIF` copre anche il caso in cui esista ma sia vuoto.
--
--    IL RIPIEGO NON È 'production'. Un database che non dichiara il proprio ambiente è un
--    database di cui non sappiamo niente, e scriverci 'production' sarebbe ripetere
--    esattamente l'errore che `ambiente.ts` documenta in testa al file: dedurre «produzione»
--    da un indizio che parla d'altro, e riempire la tabella di incidenti inventati.
--    'sql-sconosciuto' non mente: dice che a scrivere è stato il database e che nessuno gli
--    ha detto quale sia — che è un'informazione utile, mentre NULL non lo era.
ALTER TABLE public.app_log
  ALTER COLUMN ambiente
  SET DEFAULT COALESCE(NULLIF(current_setting('app.ambiente', true), ''), 'sql-sconosciuto');

-- 3. Le righe già scritte. Sono 6, sono log (nessun dato personale: `contesto` contiene solo
...
UPDATE public.app_log
   SET ambiente = COALESCE(NULLIF(current_setting('app.ambiente', true), ''), 'sql-sconosciuto')
