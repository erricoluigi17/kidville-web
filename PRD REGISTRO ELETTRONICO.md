...
- **Ottimizzazione mobile**: alle proporzioni del design il contenuto sforava i 667px e **"Accedi" finiva sotto la piega su iPhone SE**. Aggiunto un blocco `@media (max-height: 720px)` che compatta la colonna (logo 168, mascotte 168, titolo 32, passi ridotti): ora la login **entra tutta senza scroll** su 375×667, 390×844 e 430×932, mentre sui telefoni normali restano i valori del mockup. Tastiera mobile: `inputMode="email"`, `autoCapitalize="none"`, `autoCorrect="off"`, `spellCheck={false}`, `enterKeyHint` (next/go); `touch-action: manipulation` su CTA e occhio (via il doppio-tap) e `-webkit-tap-highlight-color: transparent` (come nel design).
