...
//    `documento_path` non è una cancellazione più mite, è la stessa perdita di
      ...
      from: (tabella: string) => {
        ...
        qb.delete = (opzioni?: { count?: string }) => ({
          in: (_c: string, ids: unknown) => {
            ...
            const error = tabella === 'pratiche_personale' ? h.erroreDeleteP : h.erroreDeleteA
...
/**
 * Una pratica del modulo pubblico. Il `documento_path` porta il COGNOME nel nome
 * del file, come in produzione: serve a provare che quel nome non finisce in un log.
 */
const pratica = (id: string, extra: Record<string, unknown> = {}): Record<string, unknown> => ({
  ...
  evasa_il: null,
  documento_path: `pratiche/${id}/documento-rossi.jpg`,
  ...extra,
})
...
const anagrafica = (
  ...
  utente_id,
  cessato_il: null,
  documento_path: `personale/${utente_id}/documento-rossi.jpg`,
  ...extra,
})
...
describe('POST /api/gdpr/retention-personale — prima i file, poi le righe', () => {
  ...
  it('🔴 anche l’UPDATE che azzera `documento_path` viene DOPO la rimozione del file', async () => {
    ...
    const res = await chiama()
    expect(res.status).toBe(200)
    expect(h.sequenza.map((c) => c.tipo), 'prima remove, poi update').toEqual(['remove', 'update'])
    expect(aggiornate()).toEqual(['u1'])
    expect(patchDegliUpdate(), 'l’update azzera il percorso e nient’altro').toEqual([
      { documento_path: null },
    ])
  ...
  it('🔴 e vale identico per le ANAGRAFICHE: la scansione che non esce trattiene la SUA riga', async () => {
    ...
    // dove l'operazione è un `update`, che sembra più mite e non lo è: azzerare
    // `documento_path` mentre il file è ancora lì lo rende irraggiungibile per
    // sempre, e nemmeno il giro successivo saprebbe che esiste.
    h.anagrafiche = [
  ...
  it('una pratica SENZA scansione non fa partire nessuna chiamata allo Storage', async () => {
    h.pratiche = [pratica('p1', { documento_path: null })]
    const res = await chiama()
    expect(res.status).toBe(200)
...
describe('POST /api/gdpr/retention-personale — la pratica APPROVATA e il suo fascicolo', () => {
  it('🔴 il fascicolo che scade porta via il modulo che l’ha generato', async () => {
    h.lettureP = [
      { data: [], error: null },
      { data: [{ id: 'pa1', documento_path: null }], error: null },
    ]
  ...
  it('🔴 e il file del modulo d’origine esce PRIMA della sua riga', async () => {
    ...
    h.lettureP = [
      { data: [], error: null },
      { data: [{ id: 'pa1', documento_path: 'pratiche/pa1/documento-rossi.jpg' }], error: null },
    ]
    h.anagrafiche = [fascicoloScaduto('u1', 'pa1')]
    const res = await chiama()
    expect(res.status).toBe(200)
  ...
  it('🔴 se `origine_pratica_id` è ignota e un fascicolo scade, NON si tocca niente', async () => {
    // Stesso verso di `documento_path` ignoto: «non so quale pratica appartenga a
  ...
  it('🔴 una pratica già scaduta a 90 giorni non viene cancellata DUE volte', async () => {
    ...
    h.lettureP = [
      { data: [pratica('pa1', { documento_path: null })], error: null },
      { data: [{ id: 'pa1', documento_path: null }], error: null },
    ]
    h.anagrafiche = [fascicoloScaduto('u1', 'pa1')]
    const res = await chiama()
...
describe('POST /api/gdpr/retention-personale — i tre termini', () => {
  ...
  it('una scansione già assente dalla riga non produce nessun update a vuoto', async () => {
    // `documento_path` è già NULL: il giro precedente l'ha sbiancata. Un update
    // qui non toglierebbe niente e gonfierebbe il conteggio dichiarato.
    h.anagrafiche = [anagrafica('gia-sbiancata', { cessato_il: mesiFa(14), documento_path: null })]
    const res = await chiama()
    expect(res.status).toBe(200)
    expect(soloDi('update')).toHaveLength(0)
...
describe('POST /api/gdpr/retention-personale — il taglio grossolano, che nessuno guardava', () => {
  ...
  it('🔴 un lotto al tetto si DICHIARA: «ok» su un lotto tagliato è un mezzo lavoro travestito', async () => {
    ...
    h.pratiche = Array.from({ length: tetto }, (_v, i) =>
      pratica(`p${i}`, { documento_path: null, creata_il: giorniFa(120) }),
    )
    const res = await chiama()
    expect(res.status).toBe(200)
...
describe('POST /api/gdpr/retention-personale — il DB della CI non è migrato', () => {
  ...
  it('🔴 senza `documento_path` NON si tocca NIENTE: righe tolte e scansioni lasciate è il caso peggiore', async () => {
    // Il difetto vero, non ipotetico: senza `documento_path` la seconda `select`
    // la esclude, `percorso` resta nullo, nessuna `remove()` parte — e senza il
    // blocco le righe si cancellerebbero lo stesso. Le scansioni resterebbero nel
    // bucket senza più nessuna riga che le nomini: invisibili, non cancellate.
    h.lettureP = [
      { data: null, error: { code: '42703', message: 'column pratiche_personale.documento_path does not exist' } },
      { data: [pratica('p1', { creata_il: giorniFa(200), documento_path: undefined })], error: null },
    ]
    ...
    expect(soloDi('remove')).toHaveLength(0)
    expect(battiti()[0].campi).toMatchObject({
      esito: 'documento-path-ignoto',
      documento_path_ignoto: true,
    })
  })
  ...
  it('🔴 … e vale anche quando a perdere la colonna è l’ANAGRAFICA, non la pratica', async () => {
    ...
    h.lettureA = [
      { data: null, error: { code: 'PGRST204', message: 'column anagrafica_personale.documento_path does not exist' } },
      { data: [], error: null },
    ]
    const res = await chiama()
    expect(res.status).toBe(503)
  ...
  it('…e con la sola `documento_path` assente il ritento c’è, perché il filtro regge', async () => {
    // Il verso opposto, che dimostra che la prova qui sopra misura la RAGIONE
    // giusta e non «non si ritenta mai»: se a mancare è `documento_path`, il
    // `.lt('cessato_il')` è ancora valido e la seconda lettura ha senso. (Il giro
...
describe('lock · il termine promesso e il termine applicato sono LO STESSO numero', () => {
  ...
  it('🔴 chi approva una pratica scrive `origine_pratica_id`, e RILASCIA `documento_path`', () => {
    ...
    // lo pretendeva — commento di colonna di `pratiche_personale.documento_path`:
    ...
    // rilegge `documento_path` della pratica d'origine solo al decimo anno — mentre
    ...
    for (const f of scrittori) {
      ...
      expect(
        /documento_path\s*:\s*null/.test(sorgente),
        `${nome} crea un’anagrafica del personale e non rilascia ` +
          '`pratiche_personale.documento_path` (nessun `documento_path: null`). Il contratto della ' +
          'migrazione `20260811205643` dice «un oggetto, un proprietario»: all’approvazione la ' +
          'scansione passa all’anagrafica e la colonna della pratica torna NULL. Lasciandovi una ' +
          'copia del percorso, quella pratica se la porta per DIECI ANNI — è la Fase 3 di ' +
          '`retention-personale`, che la rilegge solo al decimo anno — mentre /privacy promette ' +
          'DODICI MESI per la copia del documento d’identità: si sfora proprio il termine con la ' +
          'base giuridica più fragile, e nessuna norma impone al datore di custodire una ' +
          'fotocopia. Le due strade, e nessuna terza: (a) azzerare `documento_path` sulla pratica ' +
          'nello stesso giro in cui nasce il fascicolo; (b) se davvero si vuole che la pratica ' +
          'conservi una copia sua, dichiararlo in /privacy con il suo termine e insegnarlo a ' +
          '`retention-personale`, che oggi non lo sa.',
      ).toBe(true)
    }
  })
  ...
  it('🔴 e non promette la SOSTITUZIONE della copia finché nessuno la esegue', () => {
    ...
    // esiste una riga che scriva `anagrafica_personale.documento_path` (l'unica
    ...
    // manteneva.
    //
    // Il giorno in cui la route di approvazione sovrascriverà `documento_path` con
    // la seconda scansione di una persona, il file precedente resterà nel bucket
    // senza più nessuna riga che lo nomini — e questo job, che legge proprio
    // `documento_path`, non lo troverà MAI: invisibile, non cancellato.
    //
    ...
    expect(
      ...
      `${dichiarano.join(' e ')} promette che la copia del documento «viene sostituita, con ` +
        ...
        `non cancellata, e per sempre, perché \`retention-personale\` legge \`documento_path\` e ` +
  ...
  it.each(TRE_VOCI)(
    ...
    ({ quale, etichetta, valore, unita }) => {
      ...
      // azzerano `anagrafica_personale.documento_path` o tolgono un oggetto da
